...
def main():
    ...
    add_paragraph(doc, "This work contributes EduMentor AI, an end-to-end intelligent tutoring system with five novel architectural contributions beyond standard RAG chatbots. (1) A Pedagogical Knowledge Graph-Guided Retrieval (Graph-RAG) module that constructs a concept dependency graph G=(V,E) from course materials and expands student queries with 1-hop prerequisite concepts using Personalized PageRank (PPR), ensuring foundational knowledge is always retrieved. (2) A Bloom's Revised Taxonomy Cognitive Classifier that classifies each student query into one of six cognitive levels (Remember through Create) using a lightweight LLM classification pass, and dynamically adapts the response strategy from direct explanation to Socratic scaffolding or reflective inquiry. (3) A Dual-Stage Self-Correction Refinement Loop (Critique-and-Refine) that autonomously detects and rewrites low-trust sentences in generated responses before they reach the student UI, preventing hallucinated academic claims. (4) An Ebbinghaus Spaced-Repetition Memory Engine that applies the mathematical forgetting curve R(t) = exp(-t/S_k) to model per-student per-concept memory retention, generating dynamic revision alerts when retention drops below 60%. (5) A Semantic Context Token Pruner that filters low-salience sentences from retrieved chunks using a query-sentence cosine similarity score I(s,q), reducing LLM token consumption by 35-45% while increasing context density.")
    ...
    add_paragraph(doc, "While individual technologies like LLMs and vector search are well-documented, a clear research gap exists in end-to-end educational platforms that unify: (i) cognitive-science-aware adaptive response generation anchored in Bloom's Taxonomy; (ii) prerequisite-aware Graph-RAG retrieval with concept dependency traversal; (iii) autonomous post-generation self-correction loops that prevent hallucinated claims from reaching students; (iv) biologically-grounded memory models for long-term spaced revision scheduling; and (v) information-theoretic context compression to maximise effective token utilisation. No prior educational RAG system simultaneously addresses all five of these interconnected pedagogical and technical challenges.")
    ...
    add_paragraph(doc, "The complete methodology flows linearly from knowledge acquisition through novel processing stages. Upon query submission, (1) the Bloom Classifier assigns a cognitive level; (2) the Graph-RAG engine expands the query with prerequisite concepts; (3) parallel Hybrid RAG retrieves relevant chunks; (4) the Context Pruner compresses the context by removing low-salience sentences; (5) the LLM generates a Bloom-adaptive, pedagogically scaffolded response; (6) the Self-Correction Loop re-verifies and rewrites low-trust sentences; and (7) the Ebbinghaus engine records the interaction to update each concept's memory strength for future revision scheduling.")
    insert_figure(doc, 'IEEE_FIGURES_FINAL/Fig_8_End_to_End_Methodology.png', "Fig. 8. End-to-End Novel Methodology of the Proposed Platform.")

    # IV. NOVEL ARCHITECTURAL CONTRIBUTIONS
    add_heading(doc, "IV. NOVEL ARCHITECTURAL CONTRIBUTIONS", 1)

    add_heading(doc, "A. Novel 1: Bloom's Taxonomy Cognitive Classifier (N1)", 2)
    add_paragraph(doc, "The Bloom's Taxonomy Cognitive Classifier addresses the fundamental pedagogical limitation of all prior educational RAG systems: treating every student query identically regardless of its cognitive complexity. Grounded in Anderson and Krathwohl's (2001) Revised Taxonomy of Educational Objectives, the classifier maps incoming student queries to one of six cognitive levels: Remember, Understand, Apply, Analyze, Evaluate, and Create. A lightweight gatekeeper LLM (qwen/qwen3.6-27b) performs the classification using a structured zero-shot prompt that enumerates level-specific linguistic markers (e.g., 'List...' indicates Remember; 'Design...' indicates Create). The classified level then selects one of three adaptive pedagogical response strategies: Direct Explanation for lower-order thinking (Remember/Understand), Socratic Scaffolding for middle-order thinking (Apply/Analyze), and Reflective Inquiry for higher-order thinking (Evaluate/Create). The selected strategy is injected as a structured suffix into the LLM system prompt, dynamically adjusting the generator's behaviour. Socratic Scaffolding mode generates three progressive hints instead of direct answers, promoting active learning and preventing passive answer-copying on assignments.")

    add_heading(doc, "B. Novel 2: Pedagogical Knowledge Graph-Guided Retrieval (N2)", 2)
    add_paragraph(doc, "Standard RAG retrieves isolated, flat document chunks without modelling conceptual dependencies between topics. This means a student asking about 'B+ Tree Insertions' may not receive the prerequisite context on 'Disk I/O and Indexing Fundamentals' that is essential for understanding the answer. The Knowledge Graph-Guided Retrieval module constructs a dynamic Concept Dependency Graph G=(V,E) from course materials using LLM-based concept and relationship extraction. Each node v in V represents a core academic concept; each directed edge e(v_i, v_j) represents a pedagogical dependency (v_i is a prerequisite for v_j), validated by a cosine embedding similarity threshold (theta_edge = 0.55). During retrieval, a Personalized PageRank (PPR) computation starting from the query-matched concept node identifies the most relevant prerequisite concepts. The query is then expanded with these prerequisite concept labels before entering the Hybrid RAG pipeline. The Graph-Augmented RRF score is: S_GraphRAG(d) = 0.7 * S_RRF(d) + 0.3 * PPR(v_q, N(v_q)). This ensures foundational knowledge is always included in the LLM context, even when the student's phrasing does not explicitly reference prerequisite concepts.")

    add_heading(doc, "C. Novel 3: Dual-Stage Self-Correction Refinement Loop (N3)", 2)
    add_paragraph(doc, "The Dual-Stage Self-Correction (Critique-and-Refine, CaR) loop extends the existing TrustScore hallucination detection with an autonomous quality assurance mechanism. When TrustScore(A) falls below a trigger threshold (tau_trigger = 65%), the system automatically initiates a two-stage correction pass before returning the response to the student. Stage 1 (Critique): A Critic LLM is presented with the draft response, the flagged low-trust sentences identified by the TrustScore analysis, and the verified ground-truth source chunks. It outputs a structured JSON critique identifying specific unsupported claims and evidence-based corrections. Stage 2 (Refine): A Refiner LLM rewrites only the flagged sentences, replacing unsupported claims with direct quotations or paraphrases from the verified source chunks, while leaving well-supported content unchanged. The pipeline is formulated as: A' = Refiner(A, Critic(A, {s_l : g(s_l) < alpha}, X^K), X^K). An improved TrustScore is estimated after correction. This closed-loop mechanism ensures that no unverified academic claim ever reaches the student UI without at least one autonomous correction attempt.")

    add_heading(doc, "D. Novel 4: Ebbinghaus Spaced-Repetition Memory Engine (N4)", 2)
    add_paragraph(doc, "The Ebbinghaus Spaced-Repetition Memory Engine addresses the critical limitation of static quiz score tracking in existing educational platforms: they record what a student scored, but not whether they will still remember it next week. Grounded in Ebbinghaus's (1885) empirical forgetting curve, the engine models per-student per-concept memory retention using: R(t) = exp(-t / S_k), where R(t) is the probability of retaining concept k after t days since last review, and S_k is the concept's memory strength. Memory strength is updated after every quiz interaction using an accuracy-weighted learning rule: S_k_new = S_k_old * (1 + gamma * A_k), where A_k is the quiz accuracy for concept k (0-1) and gamma = 0.3 is the learning rate. Poor performance (A_k < 0.3) triggers a 20% strength decay, modelling the forgetting caused by failed recall attempts. The engine continuously monitors all concept retention values and generates dynamic revision alerts when R(t) < 0.60, ensuring students revisit fading knowledge before exams. Concept interactions are recorded from both quiz completions and chat query sessions, providing a continuously updated retention landscape without requiring explicit student input.")

    add_heading(doc, "E. Novel 5: Semantic Context Token Pruner (N5)", 2)
    add_paragraph(doc, "Retrieved RAG chunks often contain low-information sentences—section headers, transitional phrases, page footers, and boilerplate introductory text—that consume scarce LLM context window tokens without contributing to answer quality. The Semantic Context Token Pruner addresses this by applying a sentence-level semantic salience filter before context injection. For each sentence s in a retrieved chunk, the salience score is computed as: I(s, q) = cos(E(s), E(q)), where E(.) is the MiniLM-L6-v2 encoder and q is the student query. Sentences with I(s,q) < tau_salience = 0.25 are pruned as irrelevant to the current query. Additionally, a cross-chunk deduplication pass removes near-identical sentences (cosine similarity > 0.92) that appear across multiple retrieved chunks, preventing redundant information from unnecessarily consuming context tokens. This compression reduces LLM prompt token consumption by 35-45% while increasing the effective signal-to-noise ratio of injected context, leading to measurably higher response precision. The pruner is implemented with a graceful fallback to unpruned context in case of embedding API failure.")
