    ...
    return (
      <div key={reply._id} style={{ marginLeft: Math.min(depth * 24, 72) }}>
        <div className="flex gap-2.5 py-2 group">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5" style={{ background: reply.author?.role === 'faculty' ? 'linear-gradient(135deg, rgba(124,111,194,0.25), rgba(79,93,200,0.10))' : 'linear-gradient(135deg, rgba(79,93,200,0.10), rgba(52,168,122,0.1))' }}>
            {reply.author?.name?.[0]?.toUpperCase() || '?'}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-xs font-semibold" style={{ color: reply.author?.role === 'faculty' ? '#7c6fc2' : '#8b94e0' }}>
                {reply.author?.name}
              </span>
              <span className="text-[9px] px-1.5 py-0.5 rounded capitalize" style={{ background: reply.author?.role === 'faculty' ? 'rgba(124,111,194,0.1)' : 'rgba(79,93,200,0.08)', color: reply.author?.role === 'faculty' ? '#7c6fc2' : '#8b94e0' }}>
                {reply.author?.role}
              </span>
            ...
            <button
              ...
              className="text-[10px] text-white/25 hover:text-[#8b94e0] mt-1 opacity-0 group-hover:opacity-100 transition-all"
            >
              ↩ Reply
            </button>
          </div>
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] px-2 py-0.5 rounded-md font-medium" style={{ background: 'rgba(79,93,200,0.10)', color: '#8b94e0' }}>
                {discussion.category}
              </span>
      ...
      <div className="px-5 py-4 border-b border-white/[0.03]" style={{ background: 'rgba(255,255,255,0.015)' }}>
        <div className="flex gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0" style={{ background: 'linear-gradient(135deg, rgba(79,93,200,0.14), rgba(124,111,194,0.15))' }}>
            {discussion.author?.name?.[0]?.toUpperCase() || '?'}
          </div>
          <div>
            <div className="text-xs font-semibold text-[#8b94e0] mb-1">{discussion.author?.name}</div>
      ...
      <div className="px-5 py-3 border-t border-white/5">
        {replyingTo && (
          <div className="flex items-center gap-2 text-xs text-[#8b94e0] mb-2">
        ...
        <div className="flex gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 px-4 py-2.5 rounded-xl text-sm bg-white/5 border border-white/[0.08] text-white/90 outline-none focus:border-[#4f5dc8]/35 resize-none placeholder:text-white/20"
          />
          <button
            onClick={handleReply}
